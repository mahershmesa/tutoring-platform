...
| 11 | `20260802090011_governorate_centroids.sql` | إحداثيات مراكز الـ18 محافظة |
| 12 | `20260802090012_map_queries.sql` | عرض الدليل + دوال `search_teachers` / `map_teachers` |
...
استعلامان يستخدمان نفس البيانات بطريقتين:

| الدالة/العرض | الاستخدام | منو يظهر | الترتيب |
|--------------|-----------|----------|---------|
| `map_teachers(subject, gov)` | نقاط الخريطة | approved **و** `location_visible=true` | — |
| `search_teachers(subject, gov, stage, near_gov)` | قائمة البحث | كل approved المطابق (**يشمل المخفيين**) | الأقرب لمحافظة الطالب أولاً |
| `teacher_directory` (view) | أساس الاثنين | approved فقط (يحترم RLS) | — |

- `location_visible` يخفي **نقطة الخريطة فقط**؛ المدرّس يبقى قابلاً للإيجاد في
  البحث ويستلم إشعارات الأسئلة.
- "الأقرب" يُحسب بمسافة Haversine بين مراكز محافظتَي الطالب والمدرّس
  (`governorates.latitude/longitude`)؛ نفس المركز يُستخدم لموضع النقطة على الخريطة.
- كل الفلاتر اختيارية (`null` = تجاهل).

## الخطوات التالية المقترحة

1. إشعارات البريد (Resend/SendGrid) عند وصول سؤال جديد للمدرّس.
2. واجهة Next.js تستهلك هذه الدوال (Supabase JS `rpc('search_teachers', …)`).
